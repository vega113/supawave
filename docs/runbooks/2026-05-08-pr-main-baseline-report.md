...
| `unresolved_threads_at_merge_total` | 38 | GraphQL `reviewThreads.nodes.isResolved === false` |
| `unresolved_threads_per_pr_mean` | 0.13 | derived: `38 / 298` |
...
- **Some review threads leak past merge.** 38 unresolved threads across the
  qualifying set (mean 0.13 / PR). This is low in absolute terms but high
  enough that it is worth tracking — a future window where the per-PR mean
...
- `unresolved_threads_per_pr_mean` > **0.39** (= 3 × 0.13)
...
Raw export row counts from this run (none hit the `--limit` and the GraphQL
pagination stopped on `oldest mergedAt < WINDOW_START`):
...
| `/tmp/issue-590-review-threads.json` (GraphQL) | 300 | n/a (paginated) | no — stopped on `oldest mergedAt < 2026-04-08T00:00:00Z` after 3 pages |
...
before relying on `unresolved_threads_*` for that window.
