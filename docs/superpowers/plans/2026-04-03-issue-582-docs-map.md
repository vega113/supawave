...
**Tech Stack:** Markdown documentation, GitHub issue workflow, existing repo docs under `docs/`
...
- `docs/current-state.md` reflects the current repo state instead of describing `modernization` or Gradle as the active/canonical path.
...
- `README.md` is partly updated for SBT/Jakarta but still carries stale Vagrant and Gradle-era wording.
- `docs/current-state.md` still describes `README.md` as an entry point for Gradle usage, refers to `modernization` as the active branch, and says Gradle remains canonical.
...
- [ ] Step 4: Draft `docs/agents/README.md` for agent-facing entry points such as the current-state guide, epic index, and execution/process docs that help a new agent orient quickly.
...
- [ ] Step 3: Update `docs/current-state.md` so it describes the current repository and branch reality accurately, removes the "Gradle usage" and Gradle-canonical language, fixes nearby list drift if needed, and points readers to the new map docs.
