...
- MODIFIED `j2cl/src/main/java/org/waveprotocol/box/j2cl/compose/J2clComposeSurfaceView.java` — listen for `wave-blip-reply-requested` / `wave-blip-edit-requested` CustomEvents on `document.body`, and for each event mount a `<wavy-composer>` as a child of the originating `<wave-blip>` element. The view sets the same property set the existing `<composer-inline-reply>` carries (`available`, `targetLabel`, `draft`, `submitting`, `staleBasis`, `status`, `error`, `activeCommand`, `commandStatus`, `commandError`) plus new `replyTargetBlipId` and `mode` ("reply" | "edit"). **`<composer-inline-reply>` is preserved as the create-wave + legacy reply slot fallback** so the existing F-2.S6 gating contract is unchanged; the user-visible delivery in S1 is the new inline-at-blip `<wavy-composer>` surface. The full call-site retirement is deferred to S4 closeout to keep S1's blast radius bounded.
- MODIFIED `j2cl/lit/src/index.js` — register `wavy-composer`, `wavy-format-toolbar`, `wavy-link-modal`, `wavy-tags-row`, `wavy-wave-root-reply-trigger`.
