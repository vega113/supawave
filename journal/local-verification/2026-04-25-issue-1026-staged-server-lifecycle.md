...
- Additional PR review follow-up: fixed `fuser` parsing so the port label is not treated as a PID, and gated duplicate-start PID replacement assertions when none of `lsof`, `ss`, or `fuser` is available.
