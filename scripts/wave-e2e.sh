...
# sbt root is file("."), so reports land under top-level target/, not wave/target/
if compgen -G "target/e2eTest-reports/*.xml" > /dev/null 2>&1; then
  cp target/e2eTest-reports/*.xml "$RESULTS_DIR/"
  echo "[e2e] JUnit XML reports copied to $RESULTS_DIR/"
elif compgen -G "target/test-reports/*.xml" > /dev/null 2>&1; then
  cp target/test-reports/*.xml "$RESULTS_DIR/"
  echo "[e2e] JUnit XML reports copied to $RESULTS_DIR/"
fi
