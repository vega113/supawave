...
import com.google.wave.api.LocalWaveService;
...
/**
 * Jakarta override of the base class for robot agents that run on the WIAB server.
 *
 * <p>This version uses {@link LocalWaveService} instead of HTTP-based WaveService,
 * and does not extend HttpServlet.
 */
public abstract class AbstractBaseRobotAgent extends AbstractRobot {
   ...
   * Account store. Also wires up the {@link LocalWaveService}.
   *
  ...
  public AbstractBaseRobotAgent(Injector injector) {
    ...
    // Wire up LocalWaveService for in-JVM operation submission.
    LocalOperationSubmitter submitter = injector.getInstance(LocalOperationSubmitter.class);
    ParticipantId robotParticipant = ParticipantId.ofUnsafe(getRobotId() + "@" + waveDomain);
    setLocalWaveService(new LocalWaveService(submitter, robotParticipant, getVersion()));
