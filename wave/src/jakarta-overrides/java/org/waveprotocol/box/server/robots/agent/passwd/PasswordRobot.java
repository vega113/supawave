...
/**
 * Jakarta override of the password robot agent.
 *
 * <p>Fixes Jetty 12 {@link MultiMap} usage: uses {@code put()} instead of
 * {@code putAllValues(ImmutableMap.of(...))} which is no longer supported.
 */
...
public final class PasswordRobot extends AbstractCliRobotAgent {
  ...
  /**
   * Verifies user credentials using Jetty 12 compatible MultiMap API.
   */
  private void verifyCredentials(String password, ParticipantId participantId)
    ...
    // Jetty 12: use put() instead of putAllValues(ImmutableMap).
