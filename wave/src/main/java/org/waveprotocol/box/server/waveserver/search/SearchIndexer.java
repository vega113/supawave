...
public class SearchIndexer {
  ...
  public void unregisterSubscription(ParticipantId user, String queryHash) {
    SubscriptionKey key = new SubscriptionKey(user, queryHash);
    subscriptionRawQueries.computeIfPresent(
        key,
        (ignored, rawQuery) -> {
          userToSubscriptions.computeIfPresent(
              user,
              (ignoredUser, userSubscriptions) -> {
    ...
    subscriptionRawQueries.computeIfPresent(
        ...
        (ignored, rawQuery) -> {
          ...
          for (WaveId waveId : newWaveIds) {
            waveToSubscriptions.computeIfAbsent(waveId, ignoredWave -> ConcurrentHashMap.newKeySet())
                .add(key);
          }
          subscriptionToWaves.put(key, waveSet);
