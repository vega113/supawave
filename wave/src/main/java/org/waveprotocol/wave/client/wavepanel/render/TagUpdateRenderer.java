...
/**
 * Renderer of tags. Listens for tag add/remove events and updates the tags view.
 *
 * Ported from Wiab.pro, adapted for Apache Wave (simplified diff tracking since
 * WaveletOperationContext/segment versioning is not available upstream).
 *
...
public class TagUpdateRenderer {
  ...
  private final ObservableConversation.TagListener tagListener =
    ...
    public void onTagAdded(String tag) {
      TagsView tagsView = modelAsViewProvider.getTagsView(conversation);
      if (tagsView != null) {
        tagsView.appendTag(conversation, tag, null, false);
      }
    }
    ...
    public void onTagRemoved(String tag) {
      TagsView tagsView = modelAsViewProvider.getTagsView(conversation);
      if (tagsView != null) {
        tagsView.removeTag(conversation, tag, null, false);
      }
    }
  };
